...
/* LEDS */
const int pinLed[] = {3, 5, 6, 9, 10};


int ledIndex = 0;
int ledOrder = 1;
const int ledHighValue = 255; 
const int fadeTick = 30;
const int ledFadeValue = 2*fadeTick;
int totalLeds;
int ledValues[5] = {0, 0,0 ,0, 0};
...
void loop() {
    ...
    curTime = millis();    
    /* LEDS */
    if(curTime - ledPrevTime > ledDelay) { 
        ledPrevTime = curTime;
        for(int i = 0; i < totalLeds; i++){
          int val = ledValues[i];
          analogWrite(pinLed[i], val);         
          if(val == ledHighValue){ledValues[i] = ledFadeValue;}
          else if (val <= ledFadeValue && val > 0){ ledValues[i] -= fadeTick;}
          
        }
        //move index
        ledValues[ledIndex] = ledHighValue;
        if(ledIndex == 0 && ledOrder == -1) ledOrder = 1;
        if(ledIndex == totalLeds-1 && ledOrder) ledOrder = -1;
        ledIndex = ledIndex + ledOrder;
    }
